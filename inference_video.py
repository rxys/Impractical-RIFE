...
ten_flow_div = None
warp_grid = None
loaded_engine_key = None
engine_rebuild_attempted = set()
timestamp_tensors = {}
cached_f0 = None
cached_f1 = None
...
_engine_setup_lock = threading.RLock()

ENGINE_DIR_ENV = "SMOOTHIE_ENGINE_DIR"
MODEL_VERSION = "4.26"
TRT_WORKSPACE_BYTES = 4 * (1 << 30)
TRT_OPTIMIZATION_LEVEL = 5
DEFAULT_PAD_MULTIPLE = 64
...
def _superfast_coordinator():
    module = sys.modules.get("superfast")
    if module is None:
        return None
    required = (
        "_wait_for_load",
        "_notify_inference_started",
        "_notify_inference_failed",
        "_begin_engine_operation",
        "_finish_engine_operation",
    )
    return module if all(hasattr(module, name) for name in required) else None


def _wait_for_superfast_load():
    coordinator = _superfast_coordinator()
    if coordinator is not None:
        coordinator._wait_for_load()


def _resolve_engine_dir():
    configured = os.environ.get(ENGINE_DIR_ENV)
    if configured:
        engine_dir = Path(configured).expanduser().resolve()
    else:
        engine_dir = (Path.cwd() / "engines").resolve()
        os.environ[ENGINE_DIR_ENV] = str(engine_dir)
    engine_dir.mkdir(parents=True, exist_ok=True)
    return engine_dir


def _padded_dimensions(width, height, pad_multiple=DEFAULT_PAD_MULTIPLE):
    width = int(width)
    height = int(height)
...
def _runtime_search_entries():
    entries = {
        Path(entry).resolve()
        for entry in [*sys.path, *os.environ.get("PYTHONPATH", "").split(os.pathsep)]
    ...
    if runtime_spec and runtime_spec.submodule_search_locations:
        entries.update(Path(entry).parent.resolve() for entry in runtime_spec.submodule_search_locations)
    return entries


def _load_torch_tensorrt_runtime():
    runtime_entries = _runtime_search_entries()
    runtime_libraries = [
        path
...
def _reset_loaded_engine():
    global flow_engine, encode_engine, ten_flow_div, warp_grid, loaded_engine_key
    global cached_f0, cached_f1, cached_pair_key, previous_pair_img1, previous_pair_f1
    flow_engine = None
    encode_engine = None
    ten_flow_div = None
    warp_grid = None
    ...
    previous_pair_img1 = None
    previous_pair_f1 = None
    timestamp_tensors.clear()


def ensure_vsrife_engines(
    width,
    height,
    *,
    pad_multiple=DEFAULT_PAD_MULTIPLE,
    dimensions_are_padded=False,
    dtype=torch.float16,
):
    """Build, load, and warm the engine matching the supplied dimensions."""
    global flow_engine, encode_engine, ten_flow_div, warp_grid, loaded_engine_key

    _configure_cuda()
    if dtype != torch.float16:
        raise ValueError("VS-RIFE TensorRT engines use torch.float16")
    if dimensions_are_padded:
        padded_width, padded_height = int(width), int(height)
        if padded_width <= 0 or padded_height <= 0:
            raise ValueError("width and height must be greater than zero")
    else:
        padded_width, padded_height = _padded_dimensions(width, height, pad_multiple)

    engine_dir = _resolve_engine_dir()
    engine_key = (padded_width, padded_height, str(engine_dir), dtype)

    with _engine_setup_lock:
        if flow_engine is not None and encode_engine is not None and loaded_engine_key == engine_key:
            return flow_engine, encode_engine
        if flow_engine is not None or encode_engine is not None:
            _reset_loaded_engine()
            torch.cuda.empty_cache()

        setup_started = walltime.perf_counter()
        print(f"[trt] loading engines for {padded_width}x{padded_height}", flush=True)
        import gc
        ...
        def find_flow_engines():
            return sorted(
                ...
                for path in engine_dir.glob(
                    ...
                    f"workspace-{TRT_WORKSPACE_BYTES}_level-{TRT_OPTIMIZATION_LEVEL}.ts"
                )
        ...
        if not engine_paths:
            print(
                f"[trt] cached engines do not include {padded_width}x{padded_height}; "
            ...
            try:
                ...
                pass
            import torch_tensorrt  # noqa: F401 - registers compiler support
            ...
            print(
                "Building VS-RIFE TensorRT engines for "
            ...
            built = rife(
                ...
                trt_workspace_size=TRT_WORKSPACE_BYTES,
            ...
            _load_torch_tensorrt_runtime()

        if len(engine_paths) != 1:
            raise RuntimeError(f"Expected one matching VS-RIFE flow engine, found: {engine_paths}")
        flow_path = engine_paths[0]
        encode_path = Path(str(flow_path) + ".encode")
        if not encode_path.is_file():
            raise RuntimeError(f"Missing VS-RIFE encode engine: {encode_path}")
        ...
        try:
            ...
            rebuild_key = (str(flow_path), str(encode_path))
            if not stale or rebuild_key in engine_rebuild_attempted:
                raise
            engine_rebuild_attempted.add(rebuild_key)
            _reset_loaded_engine()
            ...
            try:
                ...
                pass
            return ensure_vsrife_engines(
                padded_width,
                padded_height,
                dimensions_are_padded=True,
                dtype=dtype,
            )
        ...
        warm_img0 = torch.zeros(
            (1, 3, padded_height, padded_width),
            device=device,
            dtype=dtype,
        )
        warm_img1 = torch.zeros_like(warm_img0)
        warm_f0 = encode_engine(warm_img0)
        warm_f1 = encode_engine(warm_img1)
        ...
        loaded_engine_key = engine_key
        action = "build" if built_now else "load"
        print(
            f"VS-RIFE TensorRT {action}+warmup: "
            f"{walltime.perf_counter() - setup_started:.2f}s"
        )
        return flow_engine, encode_engine


def _prepare_inference_engines(width, height, pad_multiple):
    coordinator = _superfast_coordinator()
    if coordinator is not None:
        coordinator._begin_engine_operation("inference")
    error = None
    try:
        return ensure_vsrife_engines(width, height, pad_multiple=pad_multiple)
    except BaseException as caught:
        error = caught
        raise
    finally:
        if coordinator is not None:
            coordinator._finish_engine_operation(error)


def get_pair_features(img0, img1):
    global cached_f0, cached_f1, cached_pair_key
    global previous_pair_img1, previous_pair_f1
    ensure_vsrife_engines(
        img0.shape[-1],
        img0.shape[-2],
        dimensions_are_padded=True,
        dtype=img0.dtype,
    ...
    if key != cached_pair_key:
        cached_f0 = previous_pair_f1 if img0 is previous_pair_img1 else encode_engine(img0)
...
def _main(argv=None):
    ...
    _prepare_inference_engines(w, h, args.pad_multiple)
...
def main(argv=None):
    coordinator = _superfast_coordinator()
    if coordinator is not None:
        coordinator._notify_inference_started()
    try:
        return _main(argv)
    except BaseException as error:
        if coordinator is not None:
            coordinator._notify_inference_failed(error)
        raise
